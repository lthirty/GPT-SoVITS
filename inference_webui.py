...
import LangSegment, re, json
import pdb
import torch

# 添加自定义模块路径
sys.path.append(r"F:\01.AI\01.AI-speeh-tts\01.AISoft\GPT-SoVITS-v2-240821\runtime\Lib\site-packages")
...
def get_ref_audios():
    ...
    return sorted([
        os.path.join(REFERENCE_AUDIO_DIR, f) 
...
os.environ["TEMP"] = tmp  # 确保独立启动时也能正确设置
print(f"当前临时目录设置为：{tmp}")  # 添加调试信息
...
with gr.Blocks(title="GPT-SoVITS WebUI") as app:
    ...
    with gr.Group():
        ...
        with gr.Row():
            ...
            with gr.Column(scale=3):
                ...
                refresh_refs = gr.Button(i18n("刷新列表"), variant="secondary")
        
        
        gr.Markdown(html_center(i18n("*请填写需要合成的目标文本和语种模式，右侧参数可不修改"),'h3'))
        with gr.Row():
        ...
        inference_button.click(
            get_tts_wav,
            [ref_audio_dropdown, prompt_text, prompt_language, text, text_language, how_to_cut, top_k, top_p, temperature, ref_text_free,speed,if_freeze,inp_refs],
            [output],
        )
    ...
    # 添加刷新功能
    refresh_refs.click(
        fn=lambda: {"choices": get_ref_audios(), "__type__": "update"},
